...
# Define a fixed start date for loading all data
FULL_DATA_START_DATE = datetime(2010, 1, 1)
FULL_DATA_START_DATE_STR = FULL_DATA_START_DATE.strftime('%Y-%m-%d')

# Date selection options
date_options = ["Completo","1A", "3A", "5A"]
...
start_date_to_filter = FULL_DATA_START_DATE

if selected_date_option == "1Y":
    start_date_to_filter = datetime.today() - timedelta(days=365)
elif selected_date_option == "3Y":
    start_date_to_filter = datetime.today() - timedelta(days=365 * 3)
elif selected_date_option == "5Y":
    start_date_to_filter = datetime.today() - timedelta(days=365 * 5)

start_date_to_filter = max(start_date_to_filter, FULL_DATA_START_DATE)
start_date_to_filter_str = start_date_to_filter.strftime('%Y-%m-%d')

with st.spinner("Carregando dados econômicos..."):
    full_data = load_data(CODES, FULL_DATA_START_DATE_STR)

if full_data.empty:
    st.warning("Não foi possível carregar os dados. Verifique sua conexão ou tente novamente mais tarde.")
else:
    # Filter data based on the selected date range
    data = full_data[full_data.index >= pd.to_datetime(start_date_to_filter_str)]
    
    if data.empty:
        st.warning(f"Não há dados disponíveis para o período selecionado a partir de {start_date_to_filter.strftime('%d/%m/%Y')}.")
    else:
